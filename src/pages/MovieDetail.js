...
export const MovieDetail = () => {
  ...
  useEffect(() => {
    async function fetchMovie() {
      ...
      const json = await response.json(`https://api.themoviedb.org/3/movie/${params.id}?api_key=aa89457cef454fcc7e674690dae34838`);
      setMovie(json);
      console.log(json);
    }
    fetchMovie();
    ...
    <main>
      <section className='flex justify-around flex-wrap py-5'>
        ...
        <div className='max-w-2xl text-gray-700 text-lg dark:text-white'>
          <h1 className='text-4xl fobt-bold my-3 text-center lg:text-left'>{movie.title}</h1>
          ...
          <div className="flex items-center">
            ...
            <span class=" text-gray-900 dark:text-white">{movie.vote_count} reviews</span>
          </div>
          <p className='my-4'>
          ...
          <p className='my-4'>
            ...
            <a href={`https://www.imdb.com/title/${movie.imdb_id}`} target="_blank">{movie.imdb_id} min</a>
          </p>
        </div>
      </section>
    </main>
